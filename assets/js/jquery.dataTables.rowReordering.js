...
(function ($) {
    ...
    $.fn.rowReordering = function (options) {
        ...
        function fnGetState(oTable, sSelector, id) {
            var tr = $("#" + id, oTable);
            var iCurrentPosition = fnGetPosition(oTable, tr);

            var iNewPosition = -1;
            var sDirection;
            var trPrevious = tr.prev(sSelector);
            if (trPrevious.length > 0) {
                iNewPosition = fnGetPosition(oTable, trPrevious);
                if (iNewPosition < iCurrentPosition) {
                    iNewPosition = iNewPosition + properties.iStep;
                }
            } else {
                var trNext = tr.next(sSelector);
                if (trNext.length > 0) {
                    iNewPosition = fnGetPosition(oTable, trNext);
        ...
        function fnGetPosition(oTable, tr) {
            if (typeof properties.iIndexColumn === "number") {
                var position = oTable.fnGetData(tr[0], properties.iIndexColumn);
            } else {
                var position = tr.data(properties.iIndexColumn);
            }

            return parseInt(position);
        }

        function fnSetPosition(oTable, tr, position) {
            if (typeof properties.iIndexColumn === "number") {
                oTable.fnUpdate(
                    position,
                    oTable.fnGetPosition(tr[0]), // get row position in current model
                    properties.iIndexColumn,
                    false
                ); // false = defer redraw until all row updates are done
            } else {
                tr.data(properties.iIndexColumn, position);
            }
        ...
        function fnMoveRows(oTable, sSelector, iCurrentPosition, iNewPosition, sDirection, id, sGroup) {
            ...
            $(oTable.fnGetNodes()).each(function () {
                if (sGroup != "" && $(this).attr("data-group") != sGroup)
                    return;
                var tr = $(this);
                var iRowPosition = fnGetPosition(oTable, tr);
                if (iStart <= iRowPosition && iRowPosition <= iEnd) {
                    if (tr[0].id == id) {
                        fnSetPosition(oTable, tr, iNewPosition);
                    } else {
                        if (sDirection == "back") {
                            fnSetPosition(oTable, tr, iRowPosition + properties.iStep);
                        } else {
                            fnSetPosition(oTable, tr, iRowPosition - properties.iStep);
                        }
                    }
